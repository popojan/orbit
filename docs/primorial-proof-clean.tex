...
\begin{abstract}
We present a complete rigorous proof of an unexpected formula: the primorial $p\#$ (product of all primes up to $p$) emerges as the denominator of a specific alternating factorial sum. For odd $m \geq 3$, the rational number
$S_m = \frac{1}{2} \sum_{k=1}^{(m-1)/2} \frac{(-1)^k \cdot k!}{2k+1}$
has denominator exactly equal to $\prod_{p \text{ prime}, p \leq m} p$ when reduced to lowest terms. This formula, initially discovered through computational exploration, revealed a hidden $p$-adic structure: each prime $p \leq m$ appears with valuation exactly 1 in the reduced denominator, despite higher powers appearing in individual terms. We establish this through elementary analysis of recurrence relations combined with Legendre's formula for factorial valuations, proving that the unreduced representation maintains the precise invariant $\nu_p(D_k) - \nu_p(N_k) = 1$ for all applicable primes.
\end{abstract}
...
When we replace the alternating sign with a constant positive sign and adjust the initial factor from $1/2$ to $1/6$, we introduce an additional factor of $3$ in the denominator. For $m \geq 9$, the systematic cancellation mechanism ensures this extra factor of $3$ is absorbed into the existing valuation structure, maintaining the invariant $\nu_p(D_k) - \nu_p(N_k) = 1$ for all primes $p \leq m$.
...
\label{rem:numerator}
While the denominator structure of the alternating sum $S_m$ exhibits the elegant primorial formula, the numerator sequence remains mysterious. Computational investigation reveals that many numerators are themselves prime numbers, but no simple closed form, recurrence relation, or generating function has been identified.

The numerator sequence begins: $-1, 23, 719, 5039, 40319, \ldots$ for $m = 3, 5, 7, 9, 11, \ldots$, where notably $23$, $719$, $5039$ are all prime. This suggests an intriguing number-theoretic structure that merits further investigation. Understanding why certain factorial combinations yield prime numerators, and characterizing the growth and distribution of these numerators, presents an interesting open problem connecting factorial sums to prime number theory.
\end{remark}
