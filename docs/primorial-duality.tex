...
\]
Then $\mathrm{Denominator}[S_m] = \Primorial(m)/2$.
\end{theorem}
...
\[
T_m = \sum_{k=1}^{\lfloor (m-1)/2 \rfloor} \frac{k!}{2k+1}
\]
Then $\mathrm{Denominator}[T_m] = \Primorial(m)/6$.
\end{theorem}
...
\item $D_{\text{red}}$ = reduced denominator (equals Primorial/c)
\item $G = \gcd(N_{\text{unred}}, D_{\text{unred}})$ = the common divisor
\end{itemize}
\end{definition}
...
The unreduced denominator equals $D_{\text{unred}} = 2 \cdot (2k+1)!!$. The odd double factorial decomposes as:
\[
(2k+1)!! = \left(\prod_{\substack{p \le 2k+1 \\ p \text{ odd prime}}} p^{\alpha_p}\right) \cdot \left(\prod_{c \in \mathcal{C}_{2k+1}} c\right)
\]
where $\alpha_p$ are appropriate exponents from repeated odd multiples of $p$.

For the alternating formula, $D_{\text{red}} = \Primorial(m)/2$ contains each odd prime to the first power. Therefore:
\[
G = \frac{D_{\text{unred}}}{D_{\text{red}}} = \frac{2 \cdot (2k+1)!!}{\Primorial(m)/2}
\]
The factor of 2 in the numerator cancels with the denominator, and the ratio of odd parts yields $2 \cdot \prod c$ by the decomposition above.

For the non-alternating formula, $D_{\text{red}} = \Primorial(m)/6$ is smaller by a factor of 3, making $G$ larger by the same factor. Computational verification confirms these formulas for all tested values.
\end{proof}
...
where:
\begin{enumerate}
\item $D_{\text{red}}$ encodes \textbf{prime structure}: $\Primorial(m)/c$ for $c \in \{2,6\}$
\item $G$ encodes \textbf{composite structure}: $c' \cdot \prod\{\text{odd composites}\}$ for $c' \in \{2,6\}$
...
Then $S_m = \frac{1}{2} \cdot \frac{N_k}{D_k}$ where $k = \lfloor(m-1)/2\rfloor$.
...
\begin{corollary}
After reduction to lowest terms, $\mathrm{Denominator}[S_m] = \Primorial(m)/2$ for odd $m \geq 3$.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:invariant} shows that each odd prime $p \leq m$ appears to exactly the first power in $D_k/\gcd(N_k, D_k)$. The factor of 2 in the unreduced denominator remains after reduction (since numerators are odd for $k \geq 1$). Multiplying by the prefactor $1/2$ yields denominators equal to the odd primorial, which is $\Primorial(m)/2$.
\end{proof}
