...
Let $S_m$ be as defined in (\ref{eq:main}). Express $S_m$ in lowest terms as $S_m = n/d$ where $\gcd(n,d) = 1$ and $d > 0$. Then for all primes $p$ with $3 \leq p \leq m$, we have
\[
\nu_p(d) = 1 \quad \text{and} \quad \nu_p(n) = 0
\]
\end{conjecture}
