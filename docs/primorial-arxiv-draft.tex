...
\]
where $\text{den}(q)$ denotes the denominator of rational $q$ in lowest terms. The formula has been verified computationally for $m$ up to 5000, yet a rigorous proof remains elusive. The central mystery is a systematic cancellation: although the individual denominators $\{2k+1\}$ contain prime powers ($9 = 3^2$, $25 = 5^2$, $27 = 3^3$, \ldots), the final reduced denominator contains only first powers of primes. We formulate this as an open problem about $p$-adic valuations and discuss possible theoretical connections.
\end{abstract}
...
This discovery arose through computational experimentation with Egyptian fraction decompositions and factorial series. While investigating various rational sum patterns, we observed that certain alternating sums exhibited denominators with remarkable structure---after reduction to lowest terms, only products of distinct primes appeared. Systematic testing revealed the formula presented here.

The formula is easily stated and computationally verified, yet understanding \emph{why} it works requires explaining a subtle cancellation mechanism. The numerators, built from factorials, systematically eliminate all higher prime powers from the naive least common multiple of the denominators. This paper presents the formula, provides computational evidence, and formulates the cancellation as a formal open problem.
...
The central mystery is this: the sequence $\{2k+1\}_{k=1}^{h}$ contains prime powers and composites:
\[
3, 5, 7, 9=3^2, 11, 13, 15=3 \times 5, 17, 19, 21=3 \times 7, 23, 25=5^2, 27=3^3, \ldots
\]

Naively, when computing $\text{LCM}(3, 5, 7, 9, 11, \ldots)$, we would retain $3^2$ from 9, $3^3$ from 27, $5^2$ from 25, and so on. Yet after summing and reducing (\ref{eq:main}) to lowest terms, the denominator contains \emph{only first powers} of primes.
...
This is the heart of the problem: proving that the numerators systematically contain exactly the right prime factors to cancel all $p^j$ with $j > 1$ through GCD reduction.
...
The formula has been verified computationally for all $m$ from 3 to 1,000,000.
...
This recurrence allows $O(m)$ arithmetic operations instead of $O(m)$ factorial computations, enabling verification at scale.

\subsection{Performance and Verification Range}

Using the iterative method:
\begin{itemize}
\item $m = 100$: $<1$ms (37-digit primorial)
\item $m = 1{,}000$: $\approx 2$ms (416-digit primorial)
\item $m = 10{,}000$: $\approx 15$ms (3{,}393-digit primorial)
\item $m = 100{,}000$: $\approx 2$s (33{,}907-digit primorial)
\item $m = 1{,}000{,}000$: $\approx 3$min (339{,}070-digit primorial)
\end{itemize}

All values have been verified against the standard primorial computation. The iterative formulation and verification scripts are publicly available:
\begin{center}
...
While the pattern is computationally robust, several theoretical questions remain:

\begin{enumerate}
\item \textbf{Rigorous proof:} Prove Conjecture~\ref{conj:main} using $p$-adic analysis or other techniques.

\item \textbf{Generating function connection:} Does this sum have an interpretation as a generating function for primorials? The structure suggests a deeper combinatorial or analytical framework.
...
We have presented a computationally verified formula expressing primorials as denominators of alternating factorial sums. The systematic cancellation of higher prime powers remains unexplained, suggesting deep number-theoretic structure. This problem is well-suited for formal proof assistants and could benefit from techniques in $p$-adic analysis, generating functions, or modular arithmetic.

We hope this discovery will inspire theoretical investigation and welcome collaboration on proving Conjecture~\ref{conj:main} or exploring the broader questions raised here.
