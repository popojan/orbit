...
Let $S_m$ be as defined in (\ref{eq:main}), and write $S_m = n/d$ where $n, d \in \mathbb{Z}$ with $d > 0$ (not necessarily in lowest terms). Then for all primes $p$ with $2 \leq p \leq m$:
\[
\nu_p(d) - \nu_p(n) = 1
\]
\end{conjecture}

This captures the essence of the primorial phenomenon: the $p$-adic valuations of numerator and denominator differ by exactly 1 for each prime $p \leq m$, which after reduction to lowest terms yields a denominator containing each prime exactly once.
