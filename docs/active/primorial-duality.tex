...
\begin{definition}
For odd $m \geq 3$, let $k = \lfloor(m-1)/2\rfloor$ and define:
\begin{align*}
\Sigma_m^{\text{alt}} &= \sum_{i=1}^{k} \frac{(-1)^i \cdot i!}{2i+1} \quad \text{(alternating factorial sum)} \\
\Sigma_m^{\text{nonalt}} &= \sum_{i=1}^{k} \frac{i!}{2i+1} \quad \text{(non-alternating factorial sum)}
\end{align*}
\end{definition}

These sums, when reduced to lowest terms, exhibit a remarkable property.

\begin{theorem}[Primorial Characterization via Factorial Sums]
\label{thm:main}
For odd integers $m \geq 3$:
\[
\mathrm{Denominator}[\Sigma_m^{\text{alt}}] = \frac{\Primorial(m)}{2}
\]
For odd integers $m \geq 9$:
\[
\mathrm{Denominator}[\Sigma_m^{\text{nonalt}}] = \frac{\Primorial(m)}{6}
\]
\end{theorem}

The proof of this theorem reveals a deeper structure: the reduction to lowest terms follows a predictable pattern involving composite numbers.

\section{The Three-Way Decomposition}

We establish the structural framework underlying both formulas.

\subsection{Unreduced vs Reduced Forms}

\begin{definition}
For each factorial sum $\Sigma_m$, write the unreduced representation as $N_{\text{unred}} / D_{\text{unred}}$ where:
\begin{itemize}
\item $N_{\text{unred}}$ = unreduced numerator (obtained by summing over common denominator)
...
For the alternating sum $\Sigma_m^{\text{alt}}$:
\[
...
For the non-alternating sum $\Sigma_m^{\text{nonalt}}$:
\[
...
\textbf{Step 1: Reduced denominator of alternating sum.}

By Theorem~\ref{thm:invariant} (the p-adic invariant), each odd prime $p \leq m$ satisfies $\nup(D_{\text{red}}) = 1$ in the reduced representation. The unreduced numerators are even (divisible by 2), so the factor of 2 from $D_{\text{unred}} = 2 \cdot (2k+1)!!$ cancels during reduction.

The reduced denominator for $\Sigma_m^{\text{alt}}$ contains only odd primes to the first power:
\[
D_{\text{red}}^{\text{alt}} = \prod_{\substack{3 \leq p \leq m \\ p \text{ prime}}} p = \frac{\Primorial(m)}{2}
\]

Note that $\Primorial(m)/2$ denotes the product of odd primes only (i.e., $\Primorial(m)$ without the factor of 2).
...
For the non-alternating sum $\Sigma_m^{\text{nonalt}}$, removing the alternating sign $(-1)^i$ affects the cancellation at $i = 1$ (where the denominator is 3). Direct computation shows:
\begin{align*}
...
The factorial sums admit a canonical three-way decomposition:
\[
\Sigma_m = \frac{N_{\text{red}}}{D_{\text{red}}} = \frac{N_{\text{red}}}{G \cdot D_{\text{red}}} \cdot G = \frac{N_{\text{unred}}}{D_{\text{unred}}}
\]
where:
\begin{enumerate}
\item $D_{\text{red}}$ encodes \textbf{prime structure}: $\Primorial(m)/2$ for alternating, $\Primorial(m)/6$ for non-alternating
\item $G$ encodes \textbf{composite structure}: $2 \cdot \prod\{\text{odd composites } \leq m\}$ for alternating, $6 \cdot \prod\{\text{odd composites } \leq m\}$ for non-alternating (for $m \geq 9$)
...
We prove Theorem~\ref{thm:main} using p-adic valuation analysis. The non-alternating case follows similarly.
...
The recurrence computes the unreduced representation of $\Sigma_m^{\text{alt}} = N_k/D_k$ where $k = \lfloor(m-1)/2\rfloor$.

\begin{remark}
For the non-alternating formula, the recurrence is identical except the numerator update omits the alternating sign:
\[
N_k^{\text{nonalt}} = N_{k-1}^{\text{nonalt}} \cdot (2k+1) + k! \cdot D_{k-1}
\]
with the same denominator recurrence. The sum is $\Sigma_m^{\text{nonalt}} = N_k^{\text{nonalt}}/D_k$.
\end{remark}
...
Theorem~\ref{thm:invariant} shows that each odd prime $p \leq m$ appears to exactly the first power in $D_k/\gcd(N_k, D_k) = D_{\text{red}}$.

For the 2-adic valuation: the unreduced numerators $N_k$ are even for $k \geq 1$ (proven by induction on the recurrence relation). Since $D_{\text{unred}} = 2 \cdot (2k+1)!!$ has $\nu_2(D_{\text{unred}}) = 1$ and $\nu_2(N_{\text{unred}}) \geq 1$, the factor of 2 cancels during reduction, giving $\nu_2(D_{\text{red}}) = 0$.

Thus the alternating sum has reduced denominator:
\[
D_{\text{red}}^{\text{alt}} = \prod_{\substack{3 \leq p \leq m \\ p \text{ prime}}} p = \frac{\Primorial(m)}{2}
\]

For the non-alternating case, the same p-adic analysis applies, except that for $m \geq 9$, the reduced denominator loses an additional factor of 3 (as shown in Step 6 of Theorem~\ref{thm:gcd}):
\[
D_{\text{red}}^{\text{nonalt}} = \frac{D_{\text{red}}^{\text{alt}}}{3} = \frac{\Primorial(m)}{6}
\]
\end{proof}
...
The numerators $N_{\text{red}}$ exhibit no discernible pattern. Many are prime, but not all. They follow no polynomial recurrence, no simple generating function, and show chaotic growth rates. This appears to be a fundamental consequence of the circular structure.
...
\section{Exact Primorial Formulas}

While Theorem~\ref{thm:main} characterizes primorials as denominators of $\Sigma_m^{\text{alt}}$ and $\Sigma_m^{\text{nonalt}}$ (up to factors of 2 and 6), we can obtain exact primorial denominators by introducing compensating prefactors.

\begin{corollary}[Exact Primorial Characterization]
\label{cor:exact}
...
\begin{proof}
Since $\mathrm{Denominator}[\Sigma_m^{\text{alt}}] = \Primorial(m)/2$, multiplying by the prefactor $1/2$ introduces an additional factor of 2 in the denominator:
\[
\mathrm{Denominator}[S_m] = \mathrm{Denominator}\left[\frac{1}{2} \cdot \Sigma_m^{\text{alt}}\right] = 2 \cdot \frac{\Primorial(m)}{2} = \Primorial(m)
\]

Similarly for the non-alternating case with prefactor $1/6$:
\[
\mathrm{Denominator}[T_m] = \mathrm{Denominator}\left[\frac{1}{6} \cdot \Sigma_m^{\text{nonalt}}\right] = 6 \cdot \frac{\Primorial(m)}{6} = \Primorial(m)
\]
\end{proof}

\begin{remark}
The prefactors $1/2$ and $1/6$ serve an aesthetic purpose: they produce denominators that are exactly $\Primorial(m)$ without any divisor. However, the essential mathematical content lies in Theorem~\ref{thm:main}, which characterizes primorials (up to small factors) via the bare factorial sums.
\end{remark}
