...
\begin{abstract}
We establish a bijection between continued fraction expansions and
Egyptian fraction decompositions. For a rational $a/b$, each pair of
consecutive CF coefficients determines one \emph{symbolic tuple}---a
compact encoding of a telescoping sum of unit fractions. This yields
$O(\log b)$ tuples for all inputs, ranging from a single tuple for
$(n-1)/n$ to $\lceil k/2 \rceil$ tuples for Fibonacci ratios $F_k/F_{k+1}$.
The bijection extends to irrationals via prefix stability.
\end{abstract}
...
\textbf{Rational} & \textbf{CF} & \textbf{Eppstein basic} & \textbf{Symbolic tuples} \\
\hline
$(n-1)/n$ & $[0; 1, n-1]$ & $n-1$ fractions & 1 tuple \\
$F_k/F_{k+1}$ & $[0; \bar{1}]$ (all 1s) & $k$ fractions & $\lceil k/2 \rceil$ tuples \\
general $a/b$ & length $n$ & $O(\sum a_i) \leq O(a)$ & $O(n/2) = O(\log b)$ \\
\end{tabular}
\end{center}
...
\begin{itemize}
\item \texttt{"List"}: Distinct unit fractions (default). Uses recursive halving
      and duplicate merging to maintain $O(\log b)$ terms even after expansion.
...
We have established an explicit bijection between continued fraction expansions
and Egyptian fraction decompositions via symbolic telescoping tuples. The main
contributions are:
\begin{enumerate}
\item A symbolic representation $(u, v, i, j)$ that compresses $O(a)$ unit fractions
      into $O(\log b)$ tuples
\item An explicit formula (Theorem~\ref{thm:bijection}) connecting tuple parameters
      to CF convergent denominators
\item The monotonicity theorem explained by paired CF differences
\item Extension to irrationals via prefix stability (Lochs bound)
\end{enumerate}
Golomb's algebraic method and Eppstein's CF method produce identical unit fraction
\emph{sequences}; the symbolic tuple representation that captures these sequences
without enumeration is our contribution.
...
\bibitem{egypt-repo}
J.~Popelka, ``egypt: Egyptian fraction decomposition via modular inverse,'' GitHub, 2023.
...
\bibitem{golomb}
S.~W.~Golomb, ``An algebraic algorithm for the representation problems of the
Ahmes Papyrus,'' \textit{Amer.\ Math.\ Monthly}, vol.~69, pp.~785--786, 1962.

\bibitem{gyimesi}
M.~Gyimesi and G.~Nyul, ``A note on Golomb's method and the continued fraction
method for Egyptian fractions,'' \textit{Annales Mathematicae et Informaticae},
vol.~42, pp.~129--134, 2013.
