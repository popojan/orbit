...
\title{The Continued Fractions--Egyptian Fractions Bijection:\\Symbolic Telescoping Representation}
...
\begin{abstract}
Gyimesi and Nyul (2013) proved that Golomb's algebraic method and
Eppstein's continued fraction method produce identical Egyptian fraction
sequences. We introduce a \emph{symbolic telescoping representation}
that compresses these sequences: each \emph{tuple} $(u,v,i,j)$ encodes
a range of $j-i+1$ consecutive unit fractions via closed-form evaluation.
This yields $O(\log b)$ tuples for any $a/b$---a compression from $O(a)$
explicit fractions to $O(\log b)$ symbols. For $(n-1)/n$, this is a single
tuple versus $n-1$ fractions.
When explicit unit fractions are required, recursive bisection expands
each tuple to $O(\log j)$ distinct fractions, giving $O(\log^2 b)$ total
output---an exponential improvement over the $O(a)$ worst case.
\end{abstract}
...
individual fractions. Gyimesi and Nyul~\cite{gyimesi} proved that Golomb's
algebraic method and the CF-based method produce \emph{identical} unit fraction
sequences---establishing equivalence but not providing compression.

We present an algorithm that produces a \emph{symbolic telescoping representation}---a
compact encoding where each tuple $(u,v,i,j)$ represents a consecutive range of
unit fractions via a closed-form sum. This is a \textbf{computational refinement}
of the known bijection, providing:

\begin{enumerate}
\item \textbf{Symbolic compression}: $O(\log b)$ tuples encode $O(a)$ unit fractions
\item \textbf{Efficient expansion}: Recursive bisection yields $O(\log^2 b)$ distinct fractions
\item \textbf{Closed-form evaluation}: Each tuple evaluates in $O(1)$ without enumeration
\item \textbf{Monotonicity guarantee}: Partial sums are strictly increasing and never overshoot
\end{enumerate}

For the extreme case $(n-1)/n$, basic CF methods enumerate $n-1$ unit fractions;
our representation uses a single tuple, expandable to $O(\log n)$ fractions via bisection.
...
We distinguish between \emph{output complexity} (number of tuples) and
\emph{computational complexity} (arithmetic operations).
...
\begin{proof}
The number of tuples equals $\lceil n/2 \rceil$ where $n$ is the CF length.
By standard results on continued fractions, $n = O(\log b)$ for any rational $a/b$.
\end{proof}

\begin{theorem}[Computational Complexity]
The modular inverse algorithm (Algorithm~\ref{alg:egypt}) computes the symbolic
representation in $O(\log b)$ iterations, each requiring one modular inverse.
The optimized single-call XGCD variant (Corollary~\ref{cor:xgcd}) achieves
$O(\log b \cdot M(\log b))$ total bit operations, where $M(k)$ is the complexity
of $k$-bit multiplication.
\end{theorem}

\begin{remark}[Comparison with Eppstein's CF Method]
Eppstein's basic continued fraction method~\cite{eppstein} produces unit fractions
by interleaving primary and secondary convergents. The output size is bounded by
``the sum of the odd terms after the first in the continued fraction list, which
is at most $a$'' (the numerator). For $\frac{n-1}{n}$ with CF $[0; 1, n-1]$,
this yields $n-1$ explicit unit fractions.

Our symbolic representation captures the \emph{same} decomposition in a single
tuple $(1, 1, 1, n-1)$, encoding the entire telescoping sum implicitly.
The tuple never needs to be expanded unless individual fractions are required.
\end{remark}

\begin{corollary}[Symbolic Compression]
For $\frac{n-1}{n}$, Eppstein's basic CF method enumerates $n-1$ unit fractions:
$\frac{1}{1\cdot 2}, \frac{1}{2\cdot 3}, \ldots, \frac{1}{(n-1)\cdot n}$.
Our algorithm produces a \emph{single} tuple:
\[
...
This symbolic compression is the key advantage: the tuple $(u, v, i, j)$ implicitly
represents $j - i + 1$ unit fractions without explicit enumeration. For applications
requiring only the sum value, closed-form evaluation (Lemma~\ref{lem:closed})
avoids expanding the tuple entirely.

\section{Connection to Continued Fractions}

The following theorem reveals an exact connection between Egyptian fractions and
...
(since even-indexed convergents approach from below and odd-indexed from above).
Pairing consecutive differences $(+, -)$ yields a positive sum equal to the
``leapfrog'' distance between even-indexed convergents.

More precisely, for convergents $c_{2k-2} < c_{2k} < q$, we have:
\[
\Delta_{2k-1} + \Delta_{2k} = (c_{2k-1} - c_{2k-2}) + (c_{2k} - c_{2k-1}) = c_{2k} - c_{2k-2}.
\]
By the leapfrog identity \eqref{eq:leapfrog} (proved in Theorem~\ref{thm:bijection}),
this difference equals $a_{2k}/(q_{2k-1} q_{2k-2})$, which matches the value of the
$k$-th Egyptian tuple $(q_{2k-2}, q_{2k-1}, 1, a_{2k})$.
\end{proof}
...
increasing. We show $S_k < q$ for all $k < n$ using the CF correspondence.

By Theorem~\ref{thm:cf}, each Egyptian tuple value equals a \emph{paired difference}
of CF convergents: $c_{2k} - c_{2k-2}$. Since CF convergents alternate around $q$
(odd convergents undershoot, even convergents overshoot), and since even-indexed
convergents form a \emph{decreasing} sequence approaching $q$ from above, we have:
\[
c_0 < c_2 < c_4 < \cdots < q < \cdots < c_5 < c_3 < c_1.
\]
The partial sum $S_k = c_{2k} - c_0$ thus satisfies $S_k < q - c_0 = q$ for all $k$
...
\begin{proposition}[XGCD = CF, {\cite[Thm.~164]{hardy-wright}}]\label{prop:xgcd}
...
\begin{corollary}[Single-Call Egypt]\label{cor:xgcd}
...
\begin{remark}
Lochs' theorem states that $\approx 0.97K$ CF terms are reliable from $K$ decimal
digits on average, but this constant varies: the golden ratio needs $\approx 2.39$
terms per digit. Since the exact bound is input-dependent, we omit truncation.
\end{remark}
...
\subsection{Golomb's Method (1962)}
Golomb~\cite{golomb} presented an algebraic algorithm based on the identity
$ps - qr = 1$ (Bezout coefficients). Gyimesi and Nyul~\cite{gyimesi} proved that
Golomb's method and the CF method produce \emph{identical} expansions.

\textbf{Complexity comparison:} Eppstein's basic CF method produces
$O(\sum_i a_i)$ unit fractions, where $a_i$ are the CF coefficients. This can be
as large as $O(a)$ (the numerator) when a single large coefficient dominates.
Our symbolic representation produces $O(n/2)$ tuples where $n$ is the CF length,
regardless of coefficient magnitudes.

\begin{center}
\begin{tabular}{l c c c c}
\textbf{Rational} & \textbf{CF} & \textbf{Eppstein basic} & \textbf{Symbolic} & \textbf{Bisected} \\
\hline
$(n-1)/n$ & $[0; 1, n-1]$ & $n-1$ fractions & 1 tuple & $O(\log n)$ fracs \\
$F_k/F_{k+1}$ & $[0; \bar{1}]$ & $k$ fractions & $\lceil k/2 \rceil$ tuples & $O(k)$ fracs \\
general $a/b$ & length $n$ & $O(a)$ & $O(\log b)$ tuples & $O(\log^2 b)$ fracs \\
\end{tabular}
\end{center}

The key insight: tuples implicitly represent ranges of unit fractions via parameters
$(u, v, i, j)$, avoiding explicit enumeration of $j - i + 1$ individual terms.
When explicit fractions are required, recursive bisection halves each tuple's value,
producing $O(\log j)$ fractions per tuple with guaranteed distinctness.

\subsection{Binary Methods}
Binary methods decompose using powers of 2. They do not provide the CF connection
or symbolic compression we achieve.

\section{Conclusion}

Gyimesi and Nyul established that Golomb's algebraic method and Eppstein's CF
method produce identical unit fraction sequences. We provide a \textbf{computational
refinement}: a symbolic telescoping representation that compresses these sequences
and enables efficient expansion. The main contributions are:
\begin{enumerate}
\item \textbf{Symbolic compression}: $O(\log b)$ tuples encode $O(a)$ unit fractions
\item \textbf{Efficient expansion}: Recursive bisection yields $O(\log^2 b)$ distinct
      fractions when explicit enumeration is required
\item \textbf{Explicit CF correspondence}: Theorem~\ref{thm:bijection} connects tuple
      parameters to CF convergent denominators
\item \textbf{Extension to irrationals}: Prefix stability with Lochs bound
\end{enumerate}
For the worst case $(n-1)/n$, this is a single tuple versus $n-1$ fractions---or
$O(\log n)$ fractions after bisection, an exponential improvement.

\subsection*{Acknowledgments}
Thanks to David Eppstein for comprehensive documentation of CF methods.
The implementation is available at~\cite{egypt-repo}.
