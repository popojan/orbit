...
\begin{theorem}[Worst-Case Guarantee]
The symbolic representation achieves $O(\log b)$ complexity for \emph{all} inputs $a/b$,
whereas convergent-enumeration methods have worst case $O(a)$ for inputs like $(n-1)/n$.
\end{theorem}

\begin{proof}
Consider $\frac{n-1}{n}$ with CF $[0; 1, n-1]$. The expanded Egyptian fraction contains
$n-1$ unit fractions: $\frac{1}{2} + \frac{1}{6} + \cdots + \frac{1}{n(n-1)}$.
Methods that enumerate individual fractions (even with post-hoc grouping) require
$O(n-1) = O(a)$ operations.

The symbolic representation produces a \emph{single} tuple $(1, 1, 1, n-1)$ directly
from the CF data, requiring only $O(\log n) = O(\log b)$ operations.

This worst-case improvement from $O(a)$ to $O(\log b)$ is the key algorithmic
contribution: direct tuple computation bypasses enumeration entirely.
\end{proof}
...
\textbf{Complexity comparison:} While Golomb/CF methods enumerate all CF convergents
individually (producing $O(n)$ unit fractions for CF length $n$), our symbolic
representation outputs $O(n/2)$ tuples directly. For extreme cases like $(n-1)/n$
with CF $[0; 1, n-1]$, traditional methods enumerate $O(n)$ convergents while
we produce a \emph{single} tuple.

\begin{center}
\begin{tabular}{l c c}
\textbf{Rational} & \textbf{Golomb (convergents)} & \textbf{Symbolic (tuples)} \\
\hline
$(n-1)/n$ & $O(n)$ & $O(1)$ \\
$F_k/F_{k+1}$ & $O(k) = O(\log b)$ & $O(k/2) = O(\log b)$ \\
general $a/b$ & $O(\log b)$ & $O(\log b)$ \\
\end{tabular}
\end{center}
