...
\begin{abstract}
We present a method for computing high-precision rational approximations to square roots by combining Pell equation solutions with nested Chebyshev polynomial iterations. The approach achieves super-quadratic convergence through a two-parameter scheme $(m_1, m_2)$ that controls both the Chebyshev polynomial order per refinement step and the iteration count, enabling trade-offs between per-step complexity and convergence rate. The method exploits cancellation of imaginary components when evaluating Chebyshev polynomial ratios at complex arguments, yielding purely rational results despite intermediate complex values. We provide optimized closed forms for low-order cases and demonstrate practical computation of 62-million-digit rational approximations, substantially exceeding continued fraction methods in computational efficiency while accepting approximately 2× denominator overhead. This reveals a fundamentally different approach to rational approximation where computational efficiency emerges from algebraic structure rather than denominator minimality.
\end{abstract}
...
Building upon earlier investigations in Egyptian fraction representations of square roots using Pell-Chebyshev methods \cite{egypt_repo}, we present a nested iteration approach that achieves substantial computational improvements. Our synthesis of classical mathematical structures yields an iterative framework with the following properties:

\begin{enumerate}
\item \textbf{Super-quadratic convergence:} The method achieves approximately 10-fold precision gain per iteration in balanced mode, increasing to 6000-fold gain in optimized configurations.
\item \textbf{Computational efficiency:} For precision requirements exceeding 200 digits, our implementation outperforms Mathematica's \texttt{Rationalize} function by factors of 3--10×.
\item \textbf{High-precision capability:} We demonstrate practical computation of a 62-million-digit rational approximation to $\sqrt{13}$.
\item \textbf{Closed-form optimizations:} We derive simplified expressions for orders $m=1$ and $m=2$, eliminating symbolic Chebyshev polynomial evaluation for these cases.
\item \textbf{Performance trade-offs:} The method incurs approximately 2× denominator overhead relative to optimal continued fraction convergents, a deliberate exchange for enhanced computational speed.
\item \textbf{Theoretical insights:} We identify connections between Pell equation solutions and Chebyshev polynomial rationality properties, suggesting unexplored relationships in classical number theory.
\end{enumerate}
...
\begin{remark}
The sextic convergence arises from a \emph{perfect cancellation} of cubic error terms when averaging Halley's method with its reciprocal. This mechanism differs fundamentally from classical iteration schemes and explains the method's exceptional performance. Note that $\sigma_1(d,n) = d/H(n)$ can be viewed as reciprocal Halley, and the symmetrization $\tau_1 = (H + d/H)/2$ precisely exploits this structure.
\end{remark}
...
We have introduced an efficient computational method for ultra-high precision rational approximation of square roots, combining Pell equation solutions with nested Chebyshev polynomial refinement. Our method achieves the following results:

\begin{itemize}
\item \textbf{Fast convergence:} Super-quadratic convergence with empirically observed 6000-fold precision gain per iteration for $\sqrt{13}$
...
\item \textbf{Algebraic insights:} Demonstrates connections between Pell equations, Chebyshev polynomials, and iterative refinement through imaginary unit cancellation in polynomial ratios
\end{itemize}
