...
The function $\alpha \mapsto C(\alpha)$ is monotone increasing and
right-continuous, with a \emph{left jump} at every rational slope
tested (Section~\ref{subsec:kinks}); at irrational points it appears
continuous~\cite{PopelkaBeattyBallot2026}.
...
\begin{remark}[The identification is a theorem]
That the limit $a_{p/q}(n)\sqrt{\pi n}/4^n$ exists and
equals~\eqref{eq:C-rational} is proven in the companion
note~\cite{regularity-note}: Theorem~0 there shows
$a_\alpha(n)/\binom{2n-1}{n-1} \to \sigma(\alpha)$ (the survival
probability of the free coin-flip gap walk) for \emph{every} real
$\alpha > 1$, with error $O(\log^2 n / n)$, via a prefix-comparison
estimate and Hoeffding's inequality for sampling without replacement;
a martingale-uniqueness argument there also proves that the boundary
system~\eqref{eq:boundary} is nonsingular and computes
$\sigma(p/q) = 1 - \rho(s_0, j_0)$.
The verification table below is therefore an implementation check.
\end{remark}
...
removing $t = 1$, it factors as $t^4 + t^3 + t^2 - 3t + 1 = 0$
(check: $t^5 - 4t^2 + 4t - 1 = (t-1)(t^4 + t^3 + t^2 - 3t + 1)$).
This quartic has two sub-unit roots, both real:
$t_1 \approx 0.43691$ and $t_2 \approx 0.71974 = \rho_0(3/2)$;
the remaining pair is complex with modulus greater than~$1$.
...
\item \textbf{Left-jump hierarchy:} at every rational slope tested,
  $C$ is continuous from the right but jumps from the left:
  $C^-(p/q) := \lim_{\alpha \to (p/q)^-} C(\alpha) < C(p/q)$, with
  jump magnitude governed by the denominator
  (Section~\ref{subsec:kinks}).
  At irrationals, the nearby jumps are small and $C$ appears
  continuous.
\end{enumerate}

The jump structure makes $C(\alpha)$ a ``devil's staircase''-like
object, though it differs from the classical staircase in two ways:
it is \emph{strictly} increasing (the classical devil's staircase is
constant on a set of full measure), and it is \emph{not continuous} ---
its singular structure includes a pure jump component supported on
the rationals.
...
% Combined and sorted, showing the left-jump structure at rationals
...
\subsection{Left jumps at rationals}\label{subsec:kinks}

At each rational slope $p/q$, the function $C(\alpha)$ is continuous
from the right but \emph{discontinuous from the left}:
\[
C^-(p/q) \;:=\; \lim_{\alpha \to (p/q)^-} C(\alpha)
\;<\; C(p/q) \;=\; \lim_{\alpha \to (p/q)^+} C(\alpha).
\]
This was established computationally (June 2026) by evaluating the
boundary system of Section~\ref{sec:rational} along the two
continued-fraction extension families
$[a_0; a_1, \ldots, a_n, K]$ and $[a_0; a_1, \ldots, a_n{-}1, 1, K]$,
which approach $p/q$ from opposite sides as $K \to \infty$, and
cross-validating the one-sided values against the lattice-path counts
themselves (agreement to ${\sim}6$ digits).%
\footnote{Verification scripts:
\texttt{docs/sessions/2026-06-11-tv-map-c-alpha-kinks/} in the
companion repository. Since $C$ is monotone, the one-sided limits
exist and do not depend on the approach family.}
Earlier drafts of this paper described these singularities as kinks
(corners); the corner appearance in coarse rational surveys is an
artifact of secant slopes taken across the left jumps.

Measured jumps $J(p/q) = C(p/q) - C^-(p/q)$:

\smallskip
\begin{center}
\begin{tabular}{lccccccc}
\toprule
$p/q$ & $3/2$ & $4/3$ & $5/3$ & $6/5$ & $7/5$ & $8/5$ & $10/7$ \\
\midrule
$C^-(p/q)$ & $0.22457$ & $0.17704$ & $0.27235$ & $0.12513$ & $0.20382$ & $0.26279$ & $0.21291$ \\
$C(p/q)$   & $0.25185$ & $0.19086$ & $0.28412$ & $0.13051$ & $0.20933$ & $0.26741$ & $0.21576$ \\
$J$        & $0.0273$  & $0.0138$  & $0.0118$  & $0.0054$  & $0.0055$  & $0.0046$  & $0.0029$ \\
$q^2 J$    & $0.109$   & $0.124$   & $0.106$   & $0.134$   & $0.138$   & $0.116$   & $0.140$ \\
\bottomrule
\end{tabular}
\end{center}
...
Three observations:
\begin{itemize}
\item \textbf{Magnitude:} on these slopes $J \approx c/q^2$ with
  $c \approx 0.11$--$0.14$, matching the approximation scale of
  continued-fraction convergents.
  The constant is \emph{word-dependent}: $J(7/5) \neq J(8/5)$ even
  though both have $q = 5$ and the same previous-convergent
  denominator.
\item \textbf{Summability constraint:} since $C$ is monotone and
  bounded, the total jump mass $\sum J(p/q)$ over any interval is
  finite, whereas $c/q^2$ summed over \emph{all} coprime $p/q$
  diverges.
  The jump law must therefore decay below $c/q^2$ for typical
  rationals (deeper continued fractions); the precise law is open.
\item \textbf{Right flatness:} a coupling argument (companion
  note~\cite{regularity-note}) gives
  $0 \leq C(p/q + \varepsilon) - C(p/q) \leq A\,e^{-\kappa/\varepsilon}$
  with explicit $\kappa > 0$: $C$ is flat to \emph{infinite order} from
  the right at every rational --- a Minkowski-question-mark-like
  flatness.
  The observed geometric decay of the one-sided difference quotients
  (per-column contraction $\mu \approx 0.93$ at $3/2$) realizes this
  bound.
\end{itemize}

At integer slopes the left limit has a closed form: numerically (to
machine precision at $k = 3, 4$, and to $1.7 \times 10^{-8}$ at
$k = 2$),
\[
C^-(k) = \frac{\tau_k - 1}{2},
...
\]
with $\tau_k$ the multinacci constant of
Section~\ref{sec:multinacci}.
Using the master-equation identity $2 - \tau_k = \tau_k^{-k}$, the
relation reads $1 - 2\,C^-(k) = \rho_k^{\,k}$ with
$\rho_k = 1/\tau_k$ the ruin probability of the $\{+k, -1\}$ walk:
the left limit is governed by the \emph{$k$-th power} of the ruin
probability, while the value itself is governed by the first power
($C(k) = 1 - \rho_k$).
Asymptotically $J(k) \sim 2^{-(k+2)}$.

The integer formula is the $q = 1$ case of a \textbf{general closed
form}.
For $x = p/q - \varepsilon$ with $\varepsilon \to 0^+$, the Beatty
allowance drops by exactly one unit, $\floor{x m} = \floor{(p/q)m} - 1$,
precisely in the columns $q \mid m$ (where $\{(p/q)m\} = 0$); the
left limit is therefore the constant of the \emph{same} periodic
Sturmian walk with a sharpened absorbing barrier --- level $0$ at
phase $j = 0$, level $-1$ at phases $j = 1, \ldots, q-1$.
Writing $\tilde\rho(s, j)$ for the ruin probability of this modified
system (same roots $t_i$ and amplitudes $A_j^{(i)}$ as
in~\eqref{eq:amplitudes}, with the phase-$0$ boundary row of the
system~\eqref{eq:boundary} replaced by
$\sum_i c_i A_0^{(i)} = 1$):
\[
C^-(p/q) \;=\; \frac{1 - \tilde\rho(s_0, j_0)}{2},
\qquad\text{hence}\qquad
J(p/q) \;=\; \frac{\tilde\rho(s_0, j_0) - \rho(s_0, j_0)}{2}
\]
is algebraic for every rational slope.
Verified against measured one-sided limits at all 25 tested
rationals, with agreement down to $6 \times 10^{-11}$ where ladder
extrapolations are sharpest ($8/5$, $5/3$).
For $q = 1$ the modified system collapses to
$\tilde\rho(s) = \rho^s$, recovering $1 - 2C^-(k) = \rho_k^k$.
Example: $J(3/2) = 0.0272922840\ldots$ has minimal polynomial
$64x^6 - 896x^5 + 4336x^4 - 7688x^3 + 2100x^2 - 528x + 13$.

At irrational $\alpha$, the same coupling argument proves continuity,
with a stretched-exponential modulus governed by the Diophantine type
of $\alpha$~\cite{regularity-note}; smoothness at irrationals is not
claimed.
