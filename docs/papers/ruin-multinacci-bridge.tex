...
slopes, the integer-slope ruin equation extends to a universal
upper bound.

\begin{proposition}[Smooth upper bound]\label{prop:upper-bound}
For all real $\alpha > 1$,
\[
C(\alpha) \leq C_{\mathrm{smooth}}(\alpha) \coloneqq 1 - \rho_0(\alpha),
\]
where $\rho_0(\alpha) \in (0,1)$ is the unique root of
$\rho^{\alpha+1} = 2\rho - 1$.
Equality holds if and only if $\alpha$ is a positive integer.
\end{proposition}

\begin{proof}[Sketch]
For integer $k$, the equation $\rho^{k+1} = 2\rho - 1$ \emph{is} the
ruin equation, so equality holds.
For rational $\alpha = p/q$ with $q \geq 2$, the smooth equation
$\rho^{(p+q)/q} = 2\rho - 1$ raises to $\rho^{p+q} = (2\rho - 1)^q$,
which is the master equation~\eqref{eq:master-intro}.
Its root $\rho_0$ is therefore a root of the master equation.
However, the master equation has $p + q - 1$ additional roots,
and the boundary system selects a linear combination that yields
$C(p/q) < 1 - \rho_0$.
The strict inequality has been verified numerically for all
rational slopes $p/q$ with $q \leq 12$.
For irrational~$\alpha$, continuity gives
$C(\alpha) = \lim C(p_k/q_k) \leq C_{\mathrm{smooth}}(\alpha)$.
\end{proof}
...
\smallskip\noindent
\textbf{Case 2: $\alpha \in [3/2, 2)$.}
By monotonicity, $C(\alpha) \geq C(3/2)$.
The palindromic minimal polynomial~\eqref{eq:C32-poly} evaluates to
$9/4096 > 0$ at $x = 1/4$, and has no real root below $C(3/2)$,
so $C(3/2) > 1/4$.
Since $1/4 = 1/2 - 2^{-2} \geq 1/2 - 2^{-\alpha}$ for $\alpha < 2$,
the bound follows.
\end{proof}

\begin{remark}[Conjectured extension to all $\alpha > 1$]
Numerical evidence strongly supports the lower bound
$C(\alpha) \geq 1/2 - 2^{-\alpha}$ for all $\alpha > 1$, with equality
only at $\alpha = 1$.
An asymptotic analysis of the master equation
$(2t-1)^q = t^{2q+1}$ for slopes $(q+1)/q$ near the phase transition
shows that $q \cdot C((q+1)/q) \to 1$ as $q \to \infty$
(the boundary system converges to a DFT, and only the principal
ruin root contributes).
Since $1 > (\ln 2)/2$, this would close the remaining gap on
$(1, 3/2)$ via a partition argument with the concavity of
$1/2 - 2^{-\alpha}$.
\end{remark}

Combined with Proposition~\ref{prop:upper-bound}, we obtain:
...
$\rho^{\alpha+1} = 2\rho - 1$.
Both bounds converge to $1/2$ exponentially as $\alpha \to \infty$;
the upper bound is tight at integer slopes, the lower bound at
$\alpha = 1$.
\end{corollary}
