...
\item \textbf{Right flatness:} the one-sided difference quotients
  from above decay geometrically, consistent with right derivative
  $0$ at rational slopes --- a Minkowski-question-mark-like flatness
  on the right side of each rational.
\end{itemize}
...
At irrational $\alpha$, the jumps of nearby rationals vanish at rate
$O(1/q^2)$, and the convergent values $C(p_k/q_k)$ approach
$C(\alpha)$ from both sides; $C$ appears continuous (no longer
claimed smooth) at irrationals.
