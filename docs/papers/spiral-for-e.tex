...
Euler discovered that~\cite{Euler1737,OEIS-A016825}
...
Since $p_n/q_n \to (3-\ee)/(\ee-1)$, we seek a M\"obius transformation that
recovers~$\ee$. A short calculation shows
\[
1 + \frac{2}{1 + \frac{3-\ee}{\ee-1}}
= 1 + \frac{2}{\frac{2}{\ee-1}}
= 1 + (\ee - 1) = \ee.
\]

This motivates the \emph{transformed convergent}
\begin{equation}\label{eq:T-def}
T_n = 1 + \frac{2}{1 + p_n/q_n}.
\end{equation}
...
The values $y_n(-2)$ form OEIS sequence A002119~\cite{OEIS-A002119}:
\[
...
is real (but transcendental):
\begin{equation}\label{eq:K-product}
K_\mu\!\left(-\tfrac{1}{2}\right) \cdot K_\nu\!\left(-\tfrac{1}{2}\right)
= -\pi\ee \cdot y_{\mu-1/2}(-2) \cdot y_{\nu-1/2}(-2).
\end{equation}

Since $y_n(-2)$ are integers, the product equals $-\pi\ee$ times an integer.
Crucially, the factor~$\pi\ee$ cancels with the numerator in~\eqref{eq:g-canonical},
yielding \emph{rational} values when both orders $2t \pm 1$ are half-integers,
i.e., for $t \in \{\frac{2k+1}{4} : k \in \Z\}$.
...
its elegant structure. The series terms are recovered via $g(\frac{3}{4} + n)$
for $n = 0, 1, 2, \ldots$
\end{remark}
...
\leavevmode
\begin{enumerate}
\item \textbf{Odd symmetry:} $g(-t) = -g(t)$ ($180^\circ$ rotation about origin).
\item \textbf{Zero at origin:} $g(0) = 0$.
\item \textbf{Decay:} $\lim_{t \to \pm\infty} g(t) = 0$.
\end{enumerate}
\end{proposition}

\subsection{Topology: a smooth closed curve with self-intersection}

The curve $\gamma: \R \cup \{\infty\} \to \C$ defined by $g$ has the following
topological structure:

\textbf{At infinity:} As $t \to +\infty$, the curve approaches the origin
along the negative real axis (angle~$\pi$). Similarly, $t \to -\infty$
approaches from the positive real axis (angle~$0$). Both branches have
tangent direction converging to~$\pi$, so they connect smoothly at infinity.

\textbf{At the origin:} We have $g(0) = 0$ with
\[
g'(0) = \frac{-16\pi\ee}{K_{-1}(-\tfrac{1}{2}) \cdot K_1(-\tfrac{1}{2})}
\approx -24.67 + 31.72i,
\]
giving tangent angle $\approx 128^\circ$.

\textbf{Self-intersection:} The origin is visited twice:
\begin{itemize}
\item At $t = 0$: tangent angle $\approx 128^\circ$
\item At $t = \pm\infty$: tangent angle $\to 180^\circ$
\end{itemize}
These are distinct tangent directions, creating a \emph{transverse
self-intersection} at the origin.

\textbf{Infinite winding:} The curve winds infinitely around the origin as
$t \to \pm\infty$. The winding rate is approximately 2 turns per unit~$t$,
with accumulated phase $\Phi(t) \approx 4\pi t$.

The image of $g$ is thus homeomorphic to a figure-eight: a smooth closed
curve with one self-intersection point.
...
see also OEIS A016825~\cite{OEIS-A016825}.
