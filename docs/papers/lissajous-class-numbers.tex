...
\usepackage{placeins}
\usepackage{hyperref}
\hypersetup{colorlinks=true, linkcolor=blue!80!black, citecolor=blue!80!black, urlcolor=blue}
\usepackage{geometry}
...
\end{align*}
These form \emph{reflection pairs}: $k'(x) + k^*(x) = p$. Both crossings have the same distance $|\sin(\pi/p)|$ from the origin, but may lie on the same side or opposite sides depending on the parity of~$x$: odd~$x$ yields same side, even~$x$ yields opposite sides (see Figures~\ref{fig:lissajous-7-11} and~\ref{fig:lissajous-2-3}).
