...
form $\Sigma\mathrm{signs} = -1 + 4(\Sigma b_2^{\text{pairs}} - \Sigma b^{\text{triple}})$; for $\omega \geq 4$,
the full hierarchy of $O(\omega \cdot 2^\omega)$ bits is required.

We prove an \emph{Inverse Parity Bias Theorem} explaining why modular inverse
parities control the sign structure: for prime $q$, the bias $\Delta(q)$ in
primitive root power parities vanishes iff $q \equiv 1 \pmod{4}$, with sign
determined by the Legendre symbol $(2|q)$ otherwise. This connects Chebyshev
geometry to quadratic reciprocity, Lehmer numbers, and Pell equation structure.
\end{abstract}
