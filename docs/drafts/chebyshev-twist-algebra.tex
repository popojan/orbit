...
\section{Motivation: The Missing Orientation}
...
computations. However, a common critique is that it loses the notion of
\emph{orientation} or \emph{twist}---the sense of ``going around'' that
angles naturally encode.

\textbf{Question:} Can we recover orientation within a rational/algebraic
framework?
...
For $n \geq 3$, the \emph{Chebyshev polygon function} traces a curve
in the complex plane as $x$ ranges over $[-1, 1]$. The curve has $n$
lobes, with vertices on the unit circle.
