...
\begin{abstract}
We present a novel method for computing ultra-high precision rational approximations to square roots of non-square integers by synthesizing Pell equation solutions with nested Chebyshev polynomial iterations. Our approach achieves remarkable super-quadratic convergence, delivering approximately 10-fold precision gain per iteration in balanced mode and up to 6000-fold in optimized configurations. The method employs a two-parameter nested refinement scheme $\nu_{m_1,m_2}$ with immediate feedback: starting from a Pell-based approximation $n_0$, we iteratively apply $m_2$ times:
\begin{equation*}
n_{k+1} = \tau_{m_1}(d, n_k) = \frac{1}{2}\left(\frac{d}{\sigma_{m_1}(d, n_k)} + \sigma_{m_1}(d, n_k)\right)
\end{equation*}
where parameter $m_1$ controls the Chebyshev polynomial order within each refinement (using ratios $U_{m_1-1}(\alpha)/U_{m_1+1}(\alpha)$ with remarkable imaginary cancellation), while $m_2$ controls the iteration count. This parameterization enables strategic trade-offs: small $m_1$ with large $m_2$ uses optimized closed forms for speed, while large $m_1$ with small $m_2$ achieves faster convergence per step. By orchestrating Pell equations and Chebyshev polynomials through iterative refinement, the method achieves practical computations exceeding 62 million digits---far beyond classical continued fraction methods---while accepting roughly 2× denominator overhead as the price for this extraordinary reach. This framework reveals a fundamentally different approach to rational approximation where computational efficiency emerges from algebraic structure rather than denominator minimality.
\end{abstract}
...
At first glance, formula \eqref{eq:sigma} presents a puzzling paradox: when $n^2 < d$ (indicating that $n$ underestimates $\sqrt{d}$), the argument to the Chebyshev polynomials becomes:
\begin{equation}
